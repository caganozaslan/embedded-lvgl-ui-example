...
void update_sysinfo_timer(lv_timer_t* timer) {
    ...
    info_texts = {
        "Cihaz: Windows Simulasyon",
        "CPU: Intel(R) Core(TM) i7",
        "Wi-Fi SSID: Simulated_WiFi",
        "IP: 192.168.1.100",
        "Saat: 2025-06-17 14:25:00",
        "Kernel: Win32 NT",
        "Uygulama: v0.1-dev"
    };
#else
    ...
    auto safe_cmd = [](const char* cmd) {
        ...
        return out.empty() ? "Veri alinamadi" : out;
        };

    info_texts = {
        "Cihaz: " + safe_cmd("cat /proc/device-tree/model"),
...
void create_system_info_screen() {
    ...
    lv_label_set_text(title, "Sistem Bilgisi");
