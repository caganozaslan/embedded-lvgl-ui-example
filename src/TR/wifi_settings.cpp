...
void update_wifi_dropdown() {
    ...
    lv_dropdown_set_options(ssid_dropdown, "Taraniyor...");
...
static void show_result_icon(bool success, const std::string& msg) {
    ...
    std::string label_text = success ? "Baglanti basarili!\n" : "Baglanti basarisiz!\n";
    label_text += msg;
    ...
    lv_timer_create([](lv_timer_t* t) {
        ...
        if (status_label && lv_obj_is_valid(status_label)) {
            lv_label_set_text(status_label, "Durum: Bekleniyor...");
        }
        lv_timer_del(t);
        }, 15000, NULL);
}
...
void create_wifi_screen() {
    ...
    lv_label_set_text(title, "Wi-Fi Ayarlari");
    lv_obj_add_style(title, &style_title, 0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 40);

    lv_obj_t* label_ssid = lv_label_create(wifi_container);
    lv_label_set_text(label_ssid, "Mevcut Aglar:");
    ...
    lv_label_set_text(label_pass, "Sifre:");
    ...
    lv_textarea_set_placeholder_text(pass_ta, "Sifreyi girin");
    ...
    lv_obj_add_event_cb(btn, [](lv_event_t* e) {
        ...
        if (status_label && lv_obj_is_valid(status_label))
            lv_label_set_text(status_label, "Baglaniyor...");
    ...
    lv_label_set_text(btn_label, "Baglan");
    ...
    lv_obj_add_event_cb(disconnect_btn, [](lv_event_t* e) {
        if (!is_wifi_connected_cached()) {
            if (status_label && lv_obj_is_valid(status_label))
                lv_label_set_text(status_label, "Zaten bagli degil.");
            return;
        }

        disconnect_wifi();
        if (status_label && lv_obj_is_valid(status_label)) {
            std::string msg = "Wi-Fi baglantisi kesildi.\nIP: Yok";
    ...
    lv_label_set_text(disconnect_lbl, "Baglantiyi Kes");
    ...
    lv_timer_create([](lv_timer_t* t) {
        ...
        if (is_wifi_connected_cached()) {
            lv_label_set_text(status_label, LV_SYMBOL_WIFI " Wi-Fi: Bagli");
            lv_obj_set_style_text_color(status_label, lv_palette_main(LV_PALETTE_GREEN), 0);
        }
        else {
            lv_label_set_text(status_label, LV_SYMBOL_CLOSE " Wi-Fi: Yok");
            lv_obj_set_style_text_color(status_label, lv_palette_main(LV_PALETTE_RED), 0);
        }
        }, 5000, NULL);
    ...
    lv_label_set_text(back_label, "Geri");
