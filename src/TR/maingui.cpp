...
void create_main_screen() {
    if (!main_container) {
        ...
        // Sol sütun
        const char* left_labels[] = { "Anlik Veriler", "Ortalama Veriler", "Sistem Bilgisi" };
        for (int i = 0; i < 3; ++i) {
        ...
        // Sağ sütun
        const char* right_labels[] = { "Wi-Fi Ayarlari", "Sensor Ayarlari", "Cihaz Ayarlari" };
        for (int i = 0; i < 3; ++i) {
            lv_obj_t* btn = lv_btn_create(main_container);
            lv_obj_set_size(btn, 140, 50);
            lv_obj_add_style(btn, &style_button, 0);
        ...
        lv_label_set_text(lbl, "Cihazi Kapat");
        lv_obj_center(lbl);
        lv_obj_add_style(lbl, &style_label_white, 0);
    }
