...
/*-------------------------------------------------
 * POPUP REFERENCES (Global, Singular)
 *------------------------------------------------*/
...
// Set System Time (for LINUX device)
static void apply_datetime(const char* date_str, const char* time_str)
{
    ...
    if (!std::regex_match(date_str, date_regex) || !std::regex_match(time_str, time_regex)) {
        show_info_popup("Date/Time format is invalid!");
        return;
    }
    ...
    if (result != 0) {
        show_info_popup("Failed to set system time!");
    }
    else {
        show_info_popup("System time set successfully.");
    }
}
#else  // For Windows simulator
static void apply_datetime(const char* date_str, const char* time_str)
{
    std::string msg = std::string("Simulation: ") + date_str + " " + time_str;
...
static void show_datetime_popup()
{
    ...
    lv_textarea_set_placeholder_text(time_ta, "HH.MM");
    ...
    lv_obj_add_event_cb(ok_btn, [](lv_event_t* e) {
        ...
        apply_datetime(date_val, time_val);
...
static void clear_all_logs()
{
    ...
    if (!out.is_open()) {
        show_info_popup("Failed to clear log file!");
        return;
    }
}
...
#else   // ____ Only for Windows build ____
static void clear_all_logs() {}
static void delete_all_wifi_records() {}
#endif
...
void create_settings_screen()
{
    if (!settings_container) {
        ...
        lv_obj_t* tab1 = lv_tabview_add_tab(tabview, "Network/Server");
        lv_obj_t* tab2 = lv_tabview_add_tab(tabview, "System");
        lv_obj_t* tab3 = lv_tabview_add_tab(tabview, "Interface");
...
static void create_network_tab(lv_obj_t* tab)
{
    ...
    lv_label_set_text(wifi_label, "Wi-Fi Auto Connect");
    ...
    lv_label_set_text(forget_label, "Forget All Wi-Fi Records");
    ...
    lv_label_set_text(server_label, "Share Data with Server");
...
static void create_system_tab(lv_obj_t* tab)
{
    ...
    lv_label_set_text(sync_label, "Synchronize");
    ...
    lv_label_set_text(clear_logs_label, "Clear Logs");
...
static void create_interface_tab(lv_obj_t* tab)
{
    ...
    lv_label_set_text(theme_label, "Day/Night Mode");
    ...
    lv_label_set_text(memory_label, "Memory Saver Mode");
    ...
    lv_label_set_text(power_label, "Low Power Mode");
...
{
    LV_UNUSED(e);
    show_confirm_popup("All Wi-Fi records will be deleted. Are you sure?", confirm_forget_wifi_cb);
}
...
{
    LV_UNUSED(e);
    show_info_popup("Time synchronized.");
}

static void check_updates_cb(lv_event_t* e)
{
    LV_UNUSED(e);
    show_info_popup("Checking for updates...");
}

static void factory_reset_cb(lv_event_t* e)
{
    LV_UNUSED(e);
    show_confirm_popup("All settings will be deleted and the device will restart. Are you sure?",
...
{
    LV_UNUSED(e);
    show_confirm_popup("All logs will be deleted. Are you sure?", confirm_clear_logs_cb);
}
...
static void confirm_forget_wifi_cb(lv_event_t* e)
{
    ...
    show_info_popup("Wi-Fi records cleared.");
}
...
static void confirm_factory_reset_cb(lv_event_t* e)
{
    ...
    show_info_popup("Restoring factory settings...");
}
...
static void confirm_clear_logs_cb(lv_event_t* e)
{
    ...
    show_info_popup("Logs cleared.");
}

/*--------- Popup helper callbacks --------*/
static void close_parent_popup_cb(lv_event_t* e)
{
...
static void show_logs_popup()
{
    ...
    if (!log_lines.empty()) {
        ...
        lv_label_set_text(empty, "No logs to display.");
        lv_obj_add_style(empty, &style_label_white, 0);
        lv_obj_align(empty, LV_ALIGN_CENTER, 0, 0);
    }
#else   // Demo logs for simulation
    const char* sample_logs[] = {
        "[Sim] Data synchronized",
        "[Sim] Sensor read successful",
        "[Sim] Sensor read error – retrying",
        "[Sim] System status: Normal",
        "[Sim] Automatic backup completed"
    };
...
static void save_settings()
{
    ...
    if (!out.is_open()) {
        show_info_popup("Could not write /etc/settings.txt!");
        return;
    }
