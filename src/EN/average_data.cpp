...
{
    // Operations that will run in a thread
    std::thread([] {
        auto data = load_data();
        ...
        // GUI operations are dispatched to the main thread
...
void create_average_data_screen()
{
    ...
    lv_label_set_text(title, "Average Data");
    ...
    lv_label_set_text(lbl_x, "Last X Samples");
    lv_obj_add_style(lbl_x, &style_label_white, 0);
    lv_obj_align(lbl_x, LV_ALIGN_TOP_MID, 0, 0);

    ta_last_x = lv_textarea_create(left_panel);
    lv_textarea_set_one_line(ta_last_x, true);
    lv_obj_clear_flag(ta_last_x, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_label_set_text(lbl_min, "Last X Minutes");
    ...
    lv_textarea_set_one_line(ta_last_min, true);
    lv_obj_clear_flag(ta_last_min, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_dropdown_set_options(dropdown_chart_type, "Line\nBar\nScatter");
    ...
    lv_checkbox_set_text(cb_temp, "Temperature");
    ...
    lv_checkbox_set_text(cb_cond, "Conductivity");
    ...
    lv_checkbox_set_text(cb_pres, "Pressure");
    ...
    lv_label_set_text(lbl_chart_btn, "Update");
    ...
    ScreenManager::get_instance().register_screen(screen, [] {/* actions when the screen is shown again */});
}
